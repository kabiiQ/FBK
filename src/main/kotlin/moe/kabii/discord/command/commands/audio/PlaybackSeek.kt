...
object PlaybackSeek : AudioCommandContainer {
    private fun trySeekCurrentTrack(origin: DiscordParameters, track: AudioTrack, target: Duration): Boolean {
        if(!track.isSeekable) {
            origin.error("The current track is not in a seekable format. (For example, streams are not seekable.)").block()
            return false
        }
        ...
        if(millis == null || millis !in 0..track.duration) {
            ...
            origin.error("The timestamp **$targetPosition** is not valid for the current track. (0:00-$endPosition)").block()
            return false
        }
    ...
    object SeekPosition : Command("seek", "position") {
        init {
            discord {
                ...
                if(args.isEmpty()) {
                    usage("**seek** is used to go to a position in a song.", "seek <timestamp>").block()
                    return@discord
                }
                ...
                if(seekTo == null) {
                    error("**$noCmd** is not a valid timestamp. Example: **seek 1:12**.").block()
                    return@discord
                }
        ...
        init {
            discord {
                ...
                val seekForwards = if(args.isEmpty()) {
                    ...
                    if(parse == null) {
                        error("**$noCmd** is not a valid length to fast-forward the track.").block()
                        return@discord
                    } else parse
                }
        ...
        init {
            discord {
                ...
                val seekBackwards = if(args.isEmpty()) {
                    ...
                    val parse = DurationParser.tryParse(noCmd)
                    if(parse == null) {
                        error("**$noCmd** is not a valid length to rewind the track.").block()
                        return@discord
                    } else parse
                }
                val position = Duration.ofMillis(track.position)
