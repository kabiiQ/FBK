            ...
            discord {
                val guildTarget = when {
                    args.isNotEmpty() -> {
                        ...
                        if (id != null) {
                            try {
                                event.client.getGuildById(id).block()
                    ...
                    guild != null -> guild  // only post personalized embed in DMs. if ran in a guild, this is the new target
                        ...
                        val mutual = BotUtil.getMutualGuilds(author).collectList().tryBlock().orNull()
                        if (mutual.isNullOrEmpty()) null
                            ...
                            val botAvatar = event.client.self.map(User::getAvatarUrl).tryBlock().orNull()
                            val prompt = embed {
                ...
                if(guildTarget == null) {
                    usage("The **setguild** command should be either: executed in the desired target guild, provided with the target guild ID, or ran in DMs and the bot will grab your mutual servers.", "setguild (guildID)").block()
                    return@discord
                }
                transaction {
                ...
                embed("Commands through PM will target the guild **${guildTarget.name}**.").block()
            }
        }
    }
}