...
object MentionRole : Command("mention", "mention-role", "role-mention", "rolemention", "mentionrole", "announce", "announcement") {
    init {
        ...
        discord {
            ...
            if(args.isEmpty()) {
                usage("", "announce <comma-separated roles to be mentioned>: <optional message>").block()
                return@discord
            }
            ...
            if(errors.isNotEmpty()) {
                ...
                usage("Could not find the following roles:\n$notFound", "mention <comma-separated role names or IDs>: <optional message>").block()
                return@discord
            }
            ...
            if(cleanRoles.isEmpty()) {
                error("Unable to mention any of the provided roles.").block()
                return@discord
            }
            ...
            val mention = if(collectRoles is Ok) collectRoles.value.joinToString(" ") else {
                error("Unable to modify the provided roles to be mentionable.").block()
                return@discord
            }
            chan.createMessage("$mention ${message ?: ""}").block()
            cleanRoles.toFlux()
