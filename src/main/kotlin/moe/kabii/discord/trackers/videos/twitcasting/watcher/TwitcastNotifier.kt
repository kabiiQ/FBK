...
abstract class TwitcastNotifier(discord: GatewayDiscordClient) : StreamWatcher(discord) {
    ...
    suspend fun createLiveNotification(info: TwitcastingMovieResponse, target: TrackedStreams.Target) {
        ...
        val chan = getChannel(guildId, target.discordChannel.channelID, FeatureChannel::twitcastingChannel, target)
        ...
        try {
            ...
            if(ce.status.code() == 403) {
                ...
                TrackerUtil.permissionDenied(chan, FeatureChannel::twitcastingChannel, target::delete)
            } else throw ce
        }
    }
}