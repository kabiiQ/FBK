...
object TwitchParser {
                ...
                try {
                    if (!response.isSuccessful) {
                        val timeout = if (response.code == 429) {
                            ...
                            LOG.error("Error calling Twitch API: $response :: ${response.body?.string()}")
                            delay(200L)
                            break
                        }
                        delay(timeout)
    ...
    suspend fun getUsers(ids: Collection<Long>): Map<Long, Result<TwitchUserInfo, StreamErr>> {
        val userLists = ids.chunked(100).map { chunk ->
            ...
            val request = Request.Builder().get().url("https://api.twitch.tv/helix/users?first=100&id=$users")
    ...
    suspend fun getUser(name: String): Result<TwitchUserInfo, StreamErr> {
        val request = Request.Builder().get().url("https://api.twitch.tv/helix/users?login=$name")
        val call = request<Helix.UserResponse>(request)
    ...
    suspend fun getStreams(ids: Collection<Long>): Map<Long, Result<TwitchStreamInfo, StreamErr>> {
        val streamLists = ids.chunked(100).map { chunk ->
            ...
            val request = Request.Builder().get().url("https://api.twitch.tv/helix/streams/?first=100&user_id=$streams")
    ...
    private suspend fun getGameInfo(id: Long): TwitchGameInfo {
        ...
        val request = Request.Builder().get().url("https://api.twitch.tv/helix/games?id=$id")
