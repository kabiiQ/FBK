...
import moe.kabii.discord.trackers.streams.twitch.TwitchErr
...
object TwitchTrackerCommand {
    suspend fun track(origin: DiscordParameters, target: String) {
        ...
        val stream = when(val lookup = TwitchParser.getUser(target)) {
            is Ok -> lookup.value
            is Err -> {
                val error = when(lookup.value) {
                    is TwitchErr.NotFound -> "Unable to find **Twitch** stream **$target**."
                    is TwitchErr.IO -> "Error tracking stream. Possible **Twitch** API issue."
                }
                origin.error(error).awaitSingle()
                return
            }
        }
        val streamId = stream.userID.toString()

        // get db 'target' object if it exists
        val dbTarget = getDBTarget(origin.chan.id, TrackedStreams.StreamInfo(TrackedStreams.DBSite.TWITCH, streamId))
            ...
            TrackedStreams.StreamChannel.find { TrackedStreams.StreamChannels.siteChannelID eq streamId }
                .elementAtOrElse(0) { _ ->
                    TrackedStreams.StreamChannel.new {
                        this.site = TrackedStreams.DBSite.TWITCH
    ...
    suspend fun untrack(origin: DiscordParameters, target: String) {
        // get stream info from username the user provides
        val stream = TwitchParser.getUser(target).orNull()

        if(stream == null) {
            origin.error("Unable to find **Twitch** stream **$target**.").awaitSingle()
            return
        }
        val streamId = stream.userID.toString()
