...
object TwitterTrackerCommand : TrackerCommand {
    ...
    override suspend fun track(origin: DiscordParameters, target: TargetArguments, features: FeatureChannel?) {
        ...
        if(!target.identifier.matches(TwitterParser.twitterUsernameRegex)) {
            origin.send(Embeds.error("Invalid Twitter username **${target.identifier}**.")).awaitSingle()
            return
        }
        ...
        if(twitterUser == null) {
            origin.send(Embeds.error("Unable to find Twitter user **${target.identifier}**.")).awaitSingle()
            return
        }
        ...
        if(existingTrack != null) {
            origin.send(Embeds.error("**Twitter/${twitterUser.username}** is already tracked in this channel.")).awaitSingle()
            return
        }
        ...
        origin.send(Embeds.fbk("Now tracking **[${twitterUser.name}](${twitterUser.url})** on Twitter!")).awaitSingle()
        if(shouldStream) {
    ...
    override suspend fun untrack(origin: DiscordParameters, target: TargetArguments) {
        ...
        propagateTransaction {
            ...
            if(existingTrack == null) {
                origin.send(Embeds.error("**Twitter/${twitterUser.username}** is not currently tracked in this channel.")).awaitSingle()
            ...
            if(
                ...
                origin.send(Embeds.fbk("No longer tracking **Twitter/${twitterUser.username}**.")).awaitSingle()
                ...
                origin.send(Embeds.error("You may not untrack **Twitter/${twitterUser.username}** unless you tracked this stream (**$tracker**) or are a channel moderator (Manage Messages permission)")).awaitSingle()
            }
        }
    }
}